...
namespace {
    ...
    auto parseText(const ParseFn& parseFn, const std::string& text) {
        Parsing2 parser(tokenize(text), parseFn);
        CHECK(parser.parse());
        return parser.parseTree;
    }

    void parseFail(const ParseFn& parseFn, const std::string& text) {
        Parsing2 parser(tokenize(text), parseFn);
