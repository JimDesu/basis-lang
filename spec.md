...
**DIRECT** &mdash; marked with `*` on the name (e.g., `*buf`); admissible only for boxed arguments. The callee operates on the caller's storage itself &mdash; no copy at entry, no restore on failure &mdash; under the no-alias license. The mode exists only in combination with the `.box` statement; &sect;6.14 is its section.
...
**Scope.** The rule governs *unobligated* region-backed runtime-length buffers only. Allocator-backed buffers are handled by the obligation system instead: ownership and escape follow &sect;10.10, the release deferred per &sect;10.11.
...
.program  .promise  .record   .scope    .splice   .stage    .static   .sub      .test
.union    .variant
```

The `.sub`, `.scope`, `.stage`, `.static`, and `.box` keywords introduce body-internal constructs &mdash; subcommands (&sect;3.12), scope blocks and their staged variant (&sect;3.17), obligation-static regions (&sect;10.16), and DIRECT extents (&sect;6.14) respectively; `.static` additionally annotates declarations immediately after their introducing keyword (&sect;10.16); `.ack` is a directive that prefixes an item at either level (&sect;3.18); the rest are top-level forms (&sect;2.2). The dot-prefix is part of the keyword token; the lexer does not produce `.` followed by a separate identifier.
