...
11. **Syntactic whitespace improves legibility.** Indentation determines block structure. The visual layout of a Basis source file reflects its operational structure; constructs at a single indentation level are siblings; constructs at deeper indentation are children. The composition rules for block markers (&sect;4.5) read off the indentation structure directly.
...
A Basis source file is a sequence of four optional sections, in order:
...
3. Zero or more `.using` directives establishing the file's standing witness selections (&sect;2.9).
4. Zero or more top-level definitions (&sect;2.2).
...
- **`.concept` *Name*` : `*body*** &mdash; Declares a concept &mdash; a single-parameter type contract enumerating commands that any member of the concept must provide. The body contains `.decl` (signature-only), `.cmd` (default-implementation), and `.operator` (operator-mapping, &sect;3.19) entries. The concept itself has no size and no allocation footprint; it is a contract, not a layout.
...
- **`.domain` *Name*` : `*ParentType*** &mdash; Declares a domain &mdash; a fixed-size buffer-backed nominal refinement of *ParentType*. The parent must reduce transitively to a fixed-size buffer-backed type. Pointers, command-typed values, objects, variants, and the range forms `[]` / `[]T` cannot be domain parents.
...
- **`.profile` *Name*` = `*entry-list*** &mdash; Declares a named bundle of witness selections. Entries are witness names (family heads or applied members) or other profile names, mixed freely and comma-separated; profiles may reference profiles, with expansion the transitive flattening and a reference cycle a static error. A profile is pure selection data &mdash; not a dispatchable entity &mdash; consumed by the `.using` directive (&sect;2.9).
...
The full lexer specification &mdash; the token classes, the literal-token grammar, the disambiguation rules between `'`, `&`, and adjacent tokens, the placement of the `.splice` modifier, and the `(T:Concept)` constraint form at method receiver positions &mdash; is given in Appendix A.
...
**Scoping.** The directive is valid at the file preamble (&sect;2.1, its canonical position &mdash; the outermost instance of the general rule) and wherever a statement is valid. A body-position `.using` scopes to its nearest enclosing scope (command frame, `.scope` block, or block-marker body), effective from its textual position to the scope's end &mdash; the local-introduction lifetime pattern. Overlapping coverage across scope levels resolves **innermost-wins**, by the same lexical walk that resolves names; the effective selection at any line is one chain-walk, not a mutation history. There are no reversal semantics: a regional override is spelled as structure, restoration falling out of scope exit:

```
.using ForwardOrd                     ; file scope: the general-purpose selection

.cmd analyze: []NumberField data =
    #summary <- (SortedSet[NumberField])        ; ForwardOrd &mdash; outer selection
    .scope
        .using ReverseOrd                        ; shadows for this region
        #ranked <- (SortedSet[NumberField])      ; ReverseOrd
        ...
...
**Generality.** The directive's mechanism is not witness-specific in principle: it references *named declarations*, which self-key (a witness fixes which pairs it covers). Any future resolution domain whose candidates are named declarations can adopt the same directive without new machinery; the `.implicit`-constructor ambiguity of &sect;7.8 is a noted candidate, contingent on those declarations becoming nameable.
...
- **DIRECT** (`*` marker on the name, e.g., `*buf`) &mdash; admissible only for boxed arguments (&sect;6.14): the callee operates on the caller's storage in place, with no copy and no restore.
...
```
min (Ord = ForwardOrd): #m, x, y                     ; regular call: prefix clause
? x :: (ForwardOrd :: before): y                     ; method call: method-prefix form
...
pairUp (Ord[K] = ChronoOrd, Ord[V] = SizeOrd): stamp, widget, #rep
pairUp (Ord[K] = ChronoOrd): stamp, widget, #rep     ; partial provision: the (V, Ord)
                                                      ; pair falls through the ordinary
                                                      ; precedence chain (&sect;9.15)
```

A qualification naming a variable the signature does not introduce, or one not bounded by the named concept, is a static error naming the mismatch. Bound witness variables (&sect;9.21) are admissible on an entry's right-hand side within a body where they are bound.
...
- `paramName <- arg` &mdash; **own**: ownership of the obligation moves into the call, and responsibility for discharging it passes to the callee. The moved-from `arg` survives as a non-owning **view** of the value on the success edge, becoming `uninit` only where the call finalizes it &mdash; the surviving-view rule is &sect;10.10, its dangling-view footgun catalogued in Appendix I (A2).
...
**Shadowing.** A `.scope` block is an ordinary nested scope, so a scope-local introduction may shadow a name visible from an enclosing scope, exactly as a block-marker body may. Name collisions *within* a single scope are forbidden (a name is bound at most once per scope, Appendix G.3), but that does not reach across the scope boundary; cross-scope shadowing is admitted (Appendix G.10). Inside the block a use resolves to the scope-local binding; the enclosing binding it hides is reachable again past the dedent.
...
**Obligation discharge at scope end.** An obligation still owned by a scope-local binding at scope end fires its discharge there (&sect;10), exactly as an obligation owned by a frame-local binding fires at frame retirement. Ownership transferred outward before scope end &mdash; by a CREATE write to an enclosing slot, a `<-` into an enclosing binding, or a by-name move into a call (&sect;10.10) &mdash; leaves the scope owning nothing for that obligation, so nothing fires at scope end; the obligation travels with its new owner. Only what the scope still owns at its close is discharged there &mdash; the scope-boundary analog of returning a resource out of the block.
...
Codes are portable program text: an `.ack "code"` in a conforming program must mean the same thing to every conforming implementation. **Caveat codes are spec-canonical** &mdash; enumerated in Appendix J, stable across revisions, spelled `area.condition` in kebab-case. Implementations may raise additional *warnings* under their own codes, but may not introduce new *caveats*: a compile-blocking tier must be spec-owned, or conforming programs stop being portable. The warning codes the specification itself names are likewise canonical and listed in Appendix J. The language's requirements are structural:

- A `.ack` must be followed by an item to prefix. A `.ack` with no subsequent item at its level &mdash; the last thing in a body, or at end of file &mdash; is a static error.
- A `.ack` that suppresses a warning the following item would not have raised is *unnecessary*, and the compiler is expected to warn that the acknowledgment is redundant or outdated. This is a warning, not an error: an acknowledgment that matches nothing is harmless to compile, but flagging it keeps acknowledgments honest as code changes.

The split is deliberate: a *structurally* incomplete `.ack` (nothing follows) cannot be given meaning and is an error, while a *semantically* redundant `.ack` (something follows, but no matching warning fires) is well-formed and merely noted.
...
For example, reading a union at a candidate the compiler cannot confirm it holds (C1, Appendix I) raises a warning; a programmer managing the discriminator themselves acknowledges it:

```
.ack "union.candidate-unchecked"   ; deliberate: we validated the tag ourselves above
#raw <- RawHeader -< packet
```

The acknowledgment encloses the single read that follows. Placed before a `.scope` block instead, it would cover every warning of that code raised anywhere in the block.

### 3.19 Operator Expressions

Basis admits an infix operator surface &mdash; the arithmetic and comparison forms the exemplars of &sect;3 and &sect;4 already use &mdash; as **pure sugar over command invocation**. Every operator application desugars to a standalone command invocation through the `#`-temporary device of principle 10 (&sect;1.2), resolving and failing exactly as the written-out call would (&sect;3.14, &sect;4). The operational semantics acquires no expression layer (Appendix D's preamble stands); this section defines the surface grammar, the desugaring, and the one declaration form &mdash; the `.operator` item &mdash; that wires a symbol to a concept method.

**The operator set and tiers.** Binding strength, tightest first; parentheses are the only override:
...
```
.concept Ord :
    .decl (S:Ord) a :: ?before: S b          ; the mapped method: an ordinary
                                             ;   concept-body signature
    .operator (<) = before                   ; HOMOGENEOUS: operand types elided;
                                             ;   the receiver is the first operand

.concept Sequence :
    .decl (S:Sequence) m :: advance: S 'out, Int64 offset
    .operator (Int64 + Sequence) = advance
                                             ; HETEROGENEOUS: operand types
                                             ;   explicit, names omitted; the
                                             ;   operand anchoring the enclosing
                                             ;   concept is the receiver -- the
                                             ;   first where possible, the
                                             ;   second where necessary
```
...
**Desugaring.** Value operators desugar innermost-first, left to right, into expression-position invocations:
...
A heterogeneous mapping reorders per its declaration &mdash; `5 + seq` desugars to `(seq :: advance: 5)`, the operand anchoring the concept serving as receiver. A test application desugars to its dispatch followed by the yield, the desugaring supplying the right operand:

```
r <- a < b                    ; desugars to:
a :: before: b                ; success/failure is the method's whole contribution
r <- b                        ; the yield, supplied by the desugaring
```

In guard position the desugared sequence forms the guard's body under &sect;4.4's grouping semantics &mdash; the guard succeeds iff every invocation succeeds:

```
? 5 < x <= y                  ; desugars to:
?  %
        5 :: before: x
        x :: atMost: y        ; the chain threads x, the first test's yield
```
...
The marks classify command-typed values uniformly across all of the language's command-bearing constructs: named `.cmd` declarations, command-typed slots (`:<...>`, `?<...>`, `!<...>`), concept methods, single- and multi-receiver methods, and lambdas (&sect;&sect;3.10&ndash;3.11, 5.14, 8). The same conformance rules apply to all.
...
Parameter modes (&sect;3.3) and parameter types are **invariant** under mark subsumption. A `:<Int'>` value is not interchangeable with `:<Int>`, `:<Int&>`, or `:<Int~>`; the subsumption relation is solely on the failure mark. Invariance is essential for soundness: the per-mode discipline at the call site (CREATE obligations, reference-initialization preconditions, READ-taint contracts, DISPOSE consume commitments) breaks if the mode is permitted to vary. The rule is developed in &sect;6.11.
...
When the new `.fail`'s payload is the bound name from a preceding `|`-with-spec, the value **moves** from its current holding frame (the recovery frame) into the new `.fail`'s originating frame (the handler's frame). The value's identity is preserved; only its holding frame changes. The dictionary for the new failure's slot is selected at the re-fail site at compile time, against the new message's bound concept &mdash; which may differ from the original message's bound concept. The static check at re-fail compares the bound payload's static concept (the engaging handler's spec concept) to the new message's payload concept: the bound concept must be a subconcept of (or equal to) the new message's concept under the Liskov-covariance rule (&sect;9.17). If the bound concept is not assignable to the new message's concept under that rule, the re-fail is a static error. The check is on the static concept only &mdash; the typechecker does not have the engaged failure's runtime concrete type at re-fail time, only the bound spec concept. The discipline composes cleanly: a chain of recover-and-re-fail handlers each performs a move into the next holding frame, until a final handler consumes the value without re-failing.
...
The split also determines whole-slot initialization tracking (&sect;6.13): a buffer-backed slot is one byte-aggregate, not a graph, and tracking it as a unit is sound because no field can have non-byte semantics. The static analyses treat each form per its category, with the guarantees of one category not silently leaking into the other.
...
A **record** is a contiguous, byte-addressable buffer with named field offsets. Record values are value-like: copyable as bytes, with no identity beyond byte-content, and laid out at deterministic offsets within their containing storage. **The layout rule: records are implicitly packed.** Fields lie in exactly the declared order with **no padding** &mdash; each field's offset is the sum of the sizes before it, deterministically, on every implementation. This is what makes a record's bytes a complete, portable account of its value (the serialization property), what gives union reinterpretation defined offsets to read at, and it puts alignment inside a record where it belongs in a systems language: in the declarer's ordering of the fields.

**Alignment: `.align`.** Alignment is never implicit; where a program needs it, it is declared, and it becomes part of the datatype. The `.align` annotation stands in a declaration's preamble &mdash; on a field, or on a `.record`, `.object`, or `.domain` type &mdash; immediately after the introducing keyword and after `.static` where present, in that fixed order. Two spellings: `.align <n>` for `n`-byte alignment, and `.align <type>` for alignment to `sizeof(type)`. The resolved alignment must be a **power of two**; any other value is a static error, with no silent rounding.

A field-level `.align` aligns the field's contents. For a compound field the alignment applies **per element** &mdash; `.align 64 [4]Counter` places elements at offsets 0, 64, 128, and 192, tail-padded to size 256 &mdash; closing false sharing at the start, the stride, *and* the tail. A type-level `.align` on a `.record` or `.object` aligns the whole contents: every field so aligned (a two-field `.align 64` record is at least 128 bytes &mdash; the cost is the semantics). A `.domain` preamble gives an aligned layout a **name**, making the distinct-datatype rule below nominal: `.domain .align 64 CacheLine : [4]Counter`.

Records remain **packed by default**; `.align` inserts exactly the deterministic padding the declaration implies, offsets computable from the page alone, and **all alignment padding is zero-filled** &mdash; `=` is byte-equality and serialization is byte-copy, so padding bytes are part of the value and must be defined. `sizeof` of an aligned type includes its padding, so containment offsets, union overlays, and the subrange divisibility rule (&sect;5.2) operate on strides with no further provision, and an aligned value's bytes remain a complete, portable account of it.

**Aligned and packed forms of one base type are different datatypes.** Implicit byte-subsumption (&sect;5.5) requires layout identity and **breaks at any layout-changing link**. Crossing is explicit, and the two directions are not symmetric. **Aligned &rarr; packed narrows by `-<`** when the base types would narrow with alignments stripped *and* the aligned payload is contiguous &mdash; leading padding only: the narrow binds a **view** of the payload bytes in place, the same byte-view-in-place category as the union row of &sect;7.14, non-owning and ceiling-bounded. Per-element-strided compounds have no contiguous payload and are outside the `-<` crossing entirely: they convert **iteratively**, element by element, in either direction. Admissibility is a static judgment on two compile-time layouts &mdash; an admissible crossing cannot fail at runtime, and an inadmissible one is a static error. An alignment-crossing `-<` view of a **boxed** slot (&sect;6.14) raises the `restrict.aliasing-hazard` caveat, exactly as `&` pointer-of does. **Packed &rarr; aligned is not a narrowing**: it is a static error at `-<`, the remedy an explicit conversion constructor (&sect;7.8) &mdash; and such layout-widening constructors may not be registered `.implicit`: a layout change never happens through a silent coercion. The declaration form is:
...
The choice not to attach a language-level discriminator to unions is structural. A discriminator on every union would force a discriminator-byte allocation cost on every union and would commit the language to a dictionary mechanism inside the buffer-backed side of the type system. Both costs are inappropriate for the lower-level coding role unions occupy: programs that want safe tagged sums use **variants** (&sect;5.12), which carry both a tag and a dictionary; programs that want byte-overlay efficiency at the cost of programmer-managed discrimination use unions. The two surfaces serve different needs, and the distinction is preserved by giving each its own discrimination model. The union/variant pair is the buffer-backed/non-buffer surface manifestation of the byte-overlay-versus-tagged-sum design choice.
...
An enum's **default value is its first entry** &mdash; the type-level default of &sect;7.10 with no declaration needed &mdash; and a constructor is free to specify a different entry.

**The concept-closing form.** The two-name constraint position may name a **concept**: `.enum StateHandling MachineState : IdleState, RunState, DoneState`. The entries are then **implementation types**, each checked at this declaration as carrying a visible witness of the concept *for its own type* (&sect;9.15's ordinary resolution). The IS-A law covers both forms uniformly: **an enum's contents are IS-A for the type the enumeration closes over** &mdash; a value-closing entry *is* a value of the constraint type, and a concept-closing enum value *is* a dispatchable value of the concept: `e :: method: args` dispatches through the active entry's dictionary. The receiver is the entry type's **canonical instance** &mdash; entry types are typically stateless, making the receiver zero-sized; whatever context a method needs rides its contract as ordinary parameters. Representation and family: **whether an enum is a domain type depends on what it closes over.** A value-closing enum remains domain-family &mdash; byte-serializable, record-legal, its bytes carrying everything. A concept-closing enum is **not** a domain type: its tag is meaningful only against the program's static per-type table, and the three-word dispatch body it denotes is not byte-serializable; it follows the non-byte-aggregate containment rules (admissible in object fields and variant candidates, not in records). The tag alone is the value &mdash; its identity for `=`, its whole semantic content &mdash; drawn against the static table with no per-value dictionary *required*. As an optimization, an implementation may copy the resolved three-word body to a fixed location co-located with the tag **at assignment time**, so dispatch reads the cached body with no table indirection; the cache is representation, never identity, and is excluded from value comparison. Concept-closing entries carry no `= literal`; auto-numbering applies invisibly.
...
A **command-typed value** is a first-class value that wraps a command-shaped operation. The language admits three constructional forms that produce command-typed values &mdash; command reference, command literal, and lambda &mdash; enumerated in &sect;3.16 and detailed in &sect;8. The type system describes command-typed values uniformly through the same family of type expressions.
...
A parameter or receiver in Basis carries one of five **modes** &mdash; **READ**, **CREATE**, **UPDATE**, **DISPOSE**, or **DIRECT** (&sect;6.14) &mdash; that determine the contract between caller and callee at that position. The modes are roles, not directions: each describes what the callee is permitted or obligated to do; the caller's side of the contract is derivable from the callee's.
...
**CREATE** &mdash; marked with `'` on the name (e.g., `'result`). The callee is statically obligated to write the parameter's slot exactly once on every successful return path &mdash; the **write-once-on-success** rule (&sect;6.12). The caller may pass either an initialized or an uninitialized slot. On success, the produced value is committed to the caller's slot via copy-restore (&sect;6.3); on failure, the caller's slot is bit-identical to its pre-call state.
...
**Named contexts** are positions where a parameter or receiver carries a name &mdash; parameter declarations, receiver declarations, lambda invoke-method parameters, capture-list entries (&sect;6.9). In a named context, the marker is a prefix on the name:
...
The call-boundary mechanics realize the per-mode contracts at the operational level. The type-system rules of &sect;&sect;6.1&ndash;6.2 and the static analyses of &sect;&sect;6.4, 6.12, 6.13 commit to behavior; the call-boundary mechanism is what makes that behavior real.

A single caller slot may be supplied to more than one parameter in one call. Binding the same slot at both a CREATE and an UPDATE position in a single call is forbidden: the resulting aliasing would defeat copy-restore failure-atomicity, since the two writes-back would race on the same storage.
...
DIRECT (`*`) bindings of boxed slots stand outside these mechanics entirely &mdash; no copy, no restore, and no working copy: a boxed slot is never statement-deferred (&sect;6.14).
...
The mechanism is enforced via a flow-sensitive **taint** property on slots and slot-views, propagated jointly with the failure-state lattice (&sect;4.13) and the initialization analysis (&sect;6.13) over the same CFG walk. The transfer-function rules:
...
### 6.7 Receiver Modes by Signature Shape
...
| Method (single- or multi-receiver) | CREATE, UPDATE, READ, DISPOSE, DIRECT (boxed receivers, &sect;6.14) | none |
...
**DIRECT &mdash; in-place on boxed storage.** The callee operates on the caller's boxed slot directly, with no copy and no restore; admissible only for boxed arguments, under the no-alias license (&sect;6.14).
...
The discipline composes with the failure-state-lattice analysis (&sect;4.13). The body's CFG is walked with both the failure-state lattice and the initialization analysis (&sect;6.13) jointly; the typechecker confirms that every path to a `clear`-state exit has performed exactly one write to each CREATE parameter. The &sect;4.13 well-formedness rule for body conformance &mdash; that conformance quantifies over reachable exits only, with non-terminating bodies vacuously conformant &mdash; applies here: a body with no reachable successful exit (a `!`-marked body, or a divergent body) imposes no CREATE-write obligation, since the universal quantification over reachable successful exits has an empty domain.
...
The resolution algorithm for a bare literal at a typed slot: at compile time, the typechecker collects `.implicit` constructors in scope whose parameter type matches the literal's source type and whose CREATE output type is at-or-above the slot's type per the buffer-backed subsumption hierarchy. If exactly one matches, the typechecker inserts the call; if multiple match at the same specificity, the resolution is ambiguous and the user must disambiguate by typed introduction (&sect;7.2). Resolution is per-call-site; no transitive `.implicit` chains are formed. (`.implicit` constructors are not currently *named* declarations; were they to become nameable, this ambiguity would fit the `.using` selection mechanism's criterion (&sect;2.9) &mdash; a noted possibility, not a commitment.)
...
- Each `?:` block's guard `# T n -< v` attempts to narrow `v` into the introduced typed local `n`. The narrow succeeds if `v`'s active candidate's type is at-or-below `T` (per the variant hierarchy rules of &sect;5.13); on success the narrowed value is bound to `n` and the body runs with that binding in scope. On mismatch, the narrow fails, the `?:` body is skipped, and the chain advances.
...
**The alignment crossing.** `# PackedType p -< alignedVal` is the one further `-<` case, specified in &sect;5.4: admissible iff the base types would narrow with alignments stripped and the aligned payload is contiguous (leading padding only); it binds a byte-view of the payload in place, the union row's category; admissibility is fully static &mdash; never a runtime failure, inadmissible is a static error &mdash; and the reverse direction is a static error whose remedy is a conversion constructor (&sect;7.8), never `.implicit`.
...
- **Atomic failure.** A failure in any subexpression aborts the whole `<-` operation. The LHS slot's pre-write state is preserved, and so is every slot the statement bound writeable: the statement's working copies (&sect;6.3) are discarded.
...
The pattern composes with the CREATE write-once rule (&sect;6.12): the Phase 2 commit is a single `<-` to `'r`, which discharges the CREATE obligation in one write. The Phase 1 locals (`contained` here) are ordinary locals, tracked individually, with their initialization states independent of `'r`'s.
...
Such storage may outlive the acquiring frame: ownership of the duty moves with the value (&sect;10.10), its release deferred to the eventual owner's lifetime end (&sect;10.11&ndash;10.12).
...
A default-region or `.scope`-local **range** (`[]` or `[]T`, &sect;5.2) carries its element data in the storage of the frame or scope that constructed it (&sect;7.20), and both `<-` (move) and `<<-` (view) bring such a value to rest by copying its *handle* &mdash; they share the element region rather than duplicating it (&sect;5.1, &sect;7.1). Resting that shared handle in a position whose lifetime ceiling exceeds the buffer's region would leave the handle pointing into storage already reclaimed when the region ends. Such a placement is a **static error**. The remedy the error names is `<<` (copy), which duplicates the elements into the destination's own storage so nothing escapes.

**Scope.** The rule governs *unobligated* region-backed ranges only. Allocator-backed buffers are handled by the obligation system instead: ownership and escape follow &sect;10.10, the release deferred per &sect;10.11.
...
**Worked case.** A `Logger` owns a range `history` field; a callee receives the logger as a borrowed `&log`, whose storage ceiling is above the callee's frame:

    .cmd record: Logger &log, Int32 n =
        # []Int32 tmp <- $[n, n, n]      ; range in THIS frame's region
...
**The ceiling bracket `'[bounder]name`.** An output parameter's CREATE mark may carry a bracket naming a binding of its own signature, declaring that the output's **lifetime ceiling is the bounder's value** &mdash; F.8's ceiling generalized from frames to values, composed along derivation chains by F.8's containment inheritance. The brackets sit immediately after the mark (&sect;4.9's pattern) and their content names a signature position (&sect;10.2's pattern): receiver, positional parameter, or implicit &mdash; every one caller-visible at the call site (&sect;3.6). An unknown name, or an empty bracket, is a static error, mirroring &sect;4.9's own bracket rules; the bracket attaches to the CREATE mark only. This serves values that derive from a receiver *without* carrying obligations &mdash; the zero-cost arena member above all (&sect;10.1: the arena itself, not its items, carries the obligation, so such an `allocate` has no obligation declaration), and interior references generally:

```
.cmd Arena a :: allocate: ^Node '[a]block, Size n
.cmd Tree t :: cursor: ^Node '[t]c        ; interior refs generally
.cmd ?copyNode: ^Node '[arena]n, Node &src / Arena arena
                                  ; the bounder need not be the receiver
```

At every store or CREATE-return of a bounded value, the destination's extent is graded against the bounder's by the &sect;10.10 lexical machinery with the bounder's extent as the bound; the undecidable case draws the `obligation.ceiling-unknown` caveat (Appendix J). A signature that exports a bounded value re-declares the bracket on its own output, enforced as a contract per &sect;4.9's discipline, so the bond crosses every frame boundary in the signature. Zero runtime words; a bounded output may also be an obligated one, both checks running independently with the sharper bound governing; a raw `^T` *copy* escaping remains Appendix I A3 territory, exactly as this section leaves its own pointer residue. Grammar delta: the `'` mark optionally takes the bracket (the CREATE-parameter production, Appendix B; a ceiling reference on the CREATE-parameter node, C.1).

**Object views grade the same way.** The store of a `<<-` view of an object is graded by the owner's extent against the destination's, both lexical (F.8 ceilings; &sect;3.17 scope-close): a destination provably outliving the owner is a static error, provable safety passes, and the pointer-mediated residue draws the `obligation.view-exceeds-owner` caveat (Appendix J) &mdash; this section's rule extended from ranges to ceiling-bounded values.

### 7.22 Finalization

**The finalizer mode `~`.** Four of the five parameter modes (&sect;6) describe how a call reads or writes a value; the fifth describes how a value *ends*. The finalizer mode **DISPOSE**, marked `~`, marks a position that consumes the value bound there: it discharges the value's finalizing duty (&sect;10) and leaves the slot `uninit`. It is the mirror of CREATE &mdash; where `'` fills an empty slot exactly once, `~` empties a full one &mdash; and completes the mode set:
...
| DIRECT | `*name` / `Type*` | stays `init` | in-place on boxed storage (&sect;6.14) |
...
**The everyday case needs no finalizer.** A frame or `.scope` slot that owns a duty fires it automatically at scope end (&sect;10.11, &sect;3.17) &mdash; the obligation system's default firing, unchanged. A thread spawned into a scope is awaited when the scope closes:

```
.promise Runtime: spawn -> join        ; spawn produces a thread confined to its spawning scope;
                                        ;   join (finalizing) awaits and reclaims it -- structured
...
**The precondition is met by construction.** A finalizing discharge requires nothing of the value's other duties, because every default is self-sufficient by declaration rule (&sect;10.2): the &sect;10.5 proviso fires each outstanding default as cleanup and then consumes, and nothing can strand. The narrowing guard `?- .obligated` remains the conservative pre-finalize test &mdash; it proves the value clear of *all* duties, useful where a program prefers explicit discharge to proviso cleanup.

**Owned-only, and its footgun.** Like the `~` parameter, `~ x` wants `x` to own its obligation. Where the binding is *provably* non-owning &mdash; a `<<-` view, or a known interior view &mdash; `~ x` is a static error (&sect;6.13). A received parameter is not such a case: ownership of an obligation is provenance, not the parameter's mode (&sect;10.2), so whether a READ or `&` parameter owns the obligation is the caller's to decide &mdash; by positional borrow versus by-name `<-` &mdash; and is undecidable in the callee. (An `&` parameter's copy-restore returns the *value* to the caller, but the obligation it carries flows independently, so `&` is no more provably-non-owning than READ.) So `~` on a parameter &mdash; or on any binding the checker cannot prove non-owning &mdash; is **allowed and unpoliced**: finalizing through a runtime non-owner strands a duty or double-frees, an acknowledged footgun on the &sect;10.3 shelf with the aliasing hazards. Owned-only is the discipline the decidable cases enforce, not a wall.
...
- **Definition-side body conformance.** The body of a command-typed value (whether a command literal or a lambda body) must conform to the value's declared failure mark, with conformance measured against the body's **exit paths** (where each reachable execution arrives at exit), not the body's internal operations:
...
### 8.6 Capture-Shadowing

**Lambda permits capture-shadowing.** A lambda body may declare a local of the same name as a captured name; the local shadows the captured name within the body's scope, per the ordinary lexical-scoping rules (Appendix G.1, shadowing at G.10). The captured name is unaffected outside the body; the lambda's invocation accesses the local where the body declares one and the captured slot where it does not.

The permission is structurally grounded: the explicit capture list enumerates the names, so the body's locals are clearly distinct from the named captures.

### 8.7 Context Variables Umbrella

The language has two mechanisms by which a command body may reference state from outside its own parameter list, collectively the **context-variables umbrella**:
...
The mechanisms compose: a lambda's capture list may bind implicit context parameters of the construction frame at construction time (&sect;8.8).
...
The rule is structurally grounded: a lambda operates as its own body with a separately-tracked capture list, so what is in scope for the body is exactly what the list names.
...
**Terminology note (for readers of earlier drafts).** Three renames are in force throughout this specification. What earlier drafts called a *class* (the `.class` contract declaration) is now a **concept** &mdash; keyword `.concept` &mdash; and *class* survives only where it names other traditions' constructs (a Haskell-style *typeclass*; *classes-in-the-OOP-sense*). What earlier drafts called an *instance* (the `.instance` declaration that a type satisfies a concept) is now a **witness** &mdash; a *named* declaration, &sect;9.4 &mdash; and the term *instance* is retired from the concept system. What earlier drafts called the runtime *witness* (the pointer in a slot's third word, the hidden dispatch parameter) is now simply the **dictionary** (or *dictionary pointer* where the pointer-ness matters); the pointed-to structure and the pointer share the one term, context distinguishing them. *Member* denotes type-membership: a type with a witness for a concept is a member of that concept.

Concepts are Basis's mechanism for declaring **type contracts** &mdash; named interfaces that a type may declare to satisfy, with each satisfaction providing concrete implementations of the contract's required commands. Concepts are not constructors and not classes-in-the-OOP-sense; the language has no class-level inheritance, no virtual methods on values, and no notion of "instance state" tied to a class. A concept is a static declaration of "what a type must support to be a member"; a witness is a static declaration of "this type supports this concept, by way of these implementations."

This section specifies the concept-and-witness declaration surfaces, the single-implementing-type discipline, the dispatch resolution sequence, the dictionary-slot model that backs dispatch at runtime, the coherence rules that govern multiple competing witness declarations across modules, and the interactions with overloading, partial application, buffer-backed-type identity, and variants.
...
**Concept names used as types are existentially qualified per appearance &mdash; with two amendments.** When a concept name (parameterized or not) appears as a parameter type in any signature, each appearance is *independently* existentially qualified &mdash; the appearances do not co-vary. Consider two signature shapes for a hypothetical standalone command:

    .decl equals: Eq a, Eq b                     ; (a)  existential per appearance
    .decl equals: (T:Eq) a, T b                  ; (b)  bound type variable

These mean different things. (a) admits `a` and `b` of any two types that each satisfy `Eq` &mdash; possibly different types from each other. (b) introduces a type variable `T` constrained to satisfy `Eq` and requires `a` and `b` to be of the *same* type. The two forms are the surface mechanisms for the existential and the bound-type-variable styles respectively, formalized as Case B and Case A at &sect;9.9. The per-appearance rule carries exactly two amendments: **the receiver position of a concept-body signature denotes the member type** (below), and **positions sharing a declared variable co-vary in type** (below, and &sect;9.13) &mdash; everywhere else, including non-receiver appearances of a concept's own name inside its own body, the rule stands verbatim.
...
.concept Ord :
    ...
    .decl (S:Ord) src :: successor: S 'out  ; member-typed CREATE output: expressible &mdash;
                                            ...
                                            ;   non-delegatable per &sect;9.8, independently)

    .decl (S:Ord) r :: register: Showable item   ; MIXED: a free existential at a non-
                                                 ;   receiver position keeps the
                                                 ;   per-appearance meaning, unchanged
```
...
.cmd Tree t :: absorbSpan: Ord lo, Ord hi = ...
    ;   an implementing method: X-positions are ordinary existential parameters &mdash;
    ;   the match is the CONTRACT's boundary obligation, not the body's
```

Dictionary entries stay monomorphic (3-word-slot parameter types, representable in the command-type grammar); when a caller passes plain values, both are boxed at one boundary and receive one dictionary (Appendix F.12's same-site selection); witness divergence arises only from pre-boxed existentials, with witness narrowing (&sect;7.14) as the spelled remedy where an algorithm needs witness agreement.
...
;   parent Ord bound explicitly; the unbound parent (Showable) resolves
;   unambiguous-or-selected-or-named AT THIS DECLARATION SITE
.witness ReverseOrdHexShow[NumberField] : OrdShow (Ord = ReverseOrd, show = hexShow)
...
- Bracket form attaches to a *header* &mdash; a concept header (concept-wide scope: all signatures share the parameter) or a witness-bearing type's header (type-wide scope, &sect;9.20) &mdash; and at a &sect;9.20 method's receiver position the bracket *restates* the type header, introducing nothing.
...
**One declaration, one member, one concept.** Each applied declaration declares that one subject type satisfies one concept (possibly a combined concept, &sect;9.2). The declaration forms:
...
- **The dynamic edge** &mdash; a concept-referential field: a pointer-to-concept field (`^StateHandling`) or a concept-typed (Case B) field. A **variant field is not admissible** as a delegation target, and naming one in a `-> field` clause is a **static error** at the declaration: an enum over `T` *is-a* `T`, but a variant over `T` is not &mdash; it *contains* one, provably only through `-<` (&sect;7.14) &mdash; so a variant delegate would smuggle a hidden narrowing, with a hidden failure path (absence included), into every caller-transparent dispatch. Selection over a closed set is the enumeration's semantic space; containment of optional payloads is the variant's. A **concept-bounded enum** field (&sect;5.9) is the third admissible kind &mdash; the **selector edge**: the entry indexes the enum's static per-type table &mdash; one table index, or none where the implementation caches the resolved body at assignment (&sect;5.9) &mdash; with no reference and no per-value dictionary required, and the field is never absent by construction (first entry is the default). For the two referential kinds, no declaration-site witness exists to resolve, because the field's member type varies at runtime by design. The delegated entries dispatch through **the dictionary carried with the field's current value**; witness selection happens at the value's *installation site* &mdash; its construction, or the statement that writes the field &mdash; unambiguous-or-selected-or-named there (&sect;9.15, Appendix F.12). The field's slot holds reference *and* dictionary and commits **as a unit** (the copied-as-a-unit discipline of &sect;9.9's 3-word slots), so a value and its governing dictionary are never observed torn:
...
The same machine, selector-form &mdash; the **recommended statechart idiom**:

  ```
  .enum StateHandling MachineState : IdleState, RunState, DoneState

  .object Machine : MachineState state, Batch work, Log journal
  .witness MachineAsState[Machine] : StateHandling -> state

  .cmd RunState s :: ?onEvent: Machine &m, Event e =
      ?- (canContinue: m, e)
          m :: state <- MachineState[DoneState]   ; the transition: an enum write
  ```

  State *data* lives in the machine's fields; state *behavior* is the selector. Never-self is meaningless (nothing points), absence is impossible (first entry is the initial state), the set is closed with `?:`-exhaustiveness available, and the transition is an ordinary domain-value write. The pointer edge above remains for open sets and separately-stored states.
...
- **With a delegation clause:** the delegate field's type must have a witness for the concept resolvable at the declaration site &mdash; *unambiguous-or-named* (&sect;9.15): if exactly one is visible it is selected; if several are, the declaration must name one via an uppercase-LHS binding entry (`Ord -> stamp (Ord = ChronoOrd)`); if none is, or the ambiguity is unnamed, compilation fails at this declaration with no fallback to other channels. The chosen edge is sealed into this witness's dictionary; chains resolve per-link, each link at its own declaration site. Concept-referential delegate fields (`^Concept` or concept-typed) have no declaration-site edge to resolve &mdash; their dispatch routes through the dictionary carried with the field's current value, selected at that value's installation site (&sect;7.11, Appendix F.12).
...
The co-location rule ensures that a witness, once declared, has a complete dictionary fixed at that point &mdash; the dictionary is not reconstructed at each use site and does not depend on the use site's import graph. Dictionaries are **canonical per declaration**: one witness declaration yields one dictionary, program-wide, and pointer identity on dictionaries is therefore decisive for witness identity &mdash; a property the annotation, unification, and narrowing machinery (&sect;9.20&ndash;&sect;9.21, &sect;7.14) relies on. Composed dictionaries for combined concepts are canonical per *tuple* of parent dictionaries (&sect;9.2).
...
The dictionary is **immutable** once constructed; witness declarations do not produce mutable state. The dictionary is referenced at dispatch sites by pointer; the dictionary mechanism (Case A or Case B) determines which dictionary the dispatch uses.
...
**Why receivers must be applied.** A method's signature lists the receiver as a structural component of its parameter list. A partial application that omits the receiver leaves a structurally incomplete reference &mdash; the method cannot be dispatched without knowing which witness to use, and the dispatch lookup is part of the method-reference machinery. The mandatory receiver binding closes this hole: every method reference has its receiver determined at construction.
...
**Resolution: unambiguous-or-selected-or-named.** Every resolution point &mdash; dispatch sites, delegation edges (&sect;9.5), combined-concept composition (&sect;9.2), and installation sites (&sect;7.11, Appendix F.12) &mdash; resolves its `(Subject, Concept)` pair through one precedence chain, most-local explicit choice first:

1. **A per-site name.** The site names a witness directly: the method-prefix form at dispatch (`x :: (ChronoOrd :: before): y`, &sect;9.16), a witness annotation at a construction (`(SortedSet[NumberField:(Ord = ForwardOrd)])`, &sect;9.20), or a prefix clause at a plain call (`min (Ord = ForwardOrd): #m, x, y`, &sect;3.14).
...
- **Cross-concept overloading.** Two distinct concepts may each declare a method with the same name. If a single type satisfies both concepts, a bare call to that name on a value of that type is ambiguous; the error names both concepts and the disambiguation forms below. Operator applications reach this rule through their desugaring (&sect;3.19): where two visible concepts map one symbol applicably for the operand types, the application is ambiguous with no precedence rule to select, the error names the candidate concepts, and the remedy line shows the desugared method form, which the user writes directly in place of the operator.
...
**Witness annotations create no overload distinctions.** Signatures differing only in witness annotations (&sect;9.20) are one overload; the annotation participates in type *checking*, never in overload *selection*. Admitting annotation-distinguished overloads would reintroduce silent selection-by-witness through the overload machinery, against the entire design of &sect;9.15.
...
```
#s <- (nf :: (Show :: show))       ; CONCEPT-selection: the Show contract's `show`;
                                   ;   the witness for (NumericField, Show) then
...
### 9.20 Witness-Bearing Parameterized Types
...
; ---- module M1 ----
.using ForwardOrd
#s <- (SortedSet[NumberField])        ; ForwardOrd selected HERE, embedded
s :: insert: x

; ---- module M2 ----
.using ReverseOrd                     ; irrelevant to s
? s :: contains: y                    ; dispatches through s's embedded ForwardOrd &mdash;
    ...                               ;   correct by dataflow; M2 cannot get this wrong
```

**Bound implies embed.** The bound and the embedding are one commitment &mdash; a header bound is usable in method bodies only because the receiver's cell is the dictionary source &mdash; and the cost (one pointer word per unpinned bounded parameter) is paid uniformly. Types not wanting per-value witness freedom pin the bound instead (&sect;9.21), which stamps no cell.

**Buffer-backed parameterized types** admit the bound as a construction-time constraint only &mdash; the containment rule (&sect;5.1) is absolute: no pointer inhabits a record, and byte-copyability with traversal-free reclamation is load-bearing. A buffer-backed witness-bearing type carries its witness *in the type*, not in the value: see &sect;9.21's mandatory-static component, under which the buffer-backed story is fully static end-to-end.
...
.cmd printAll: SortedSet[T:ShowOrd] s =   ; standalone; T introduced at first occurrence
    ...
    s :: insert: sentinel                 ; receiver-routed: s's cell decides
```

`T:ShowOrd` satisfies the header's `T:Ord` demand because the composed dictionary's identity *is* the tuple of parent dictionaries (&sect;9.2) &mdash; projecting the `Ord` component is canonical and yields the identical dictionary a direct bound would flow. Pay-per-use is preserved: constructing `SortedSet[Widget]` demands only the header pair; only `printAll:` call sites price `(Widget, ShowOrd)`, componentwise. Inside such a command, the flowed bound's `Ord` component and a parameter's cell are **two live channels for one pair**, and the channel rule of &sect;9.21 governs: a bare dispatch on a multi-channel pair is a static ambiguity error, escaped by unification, narrowing, or receiver-routing.

### 9.21 The Static Witness Component

Every installation site selects its witness at compile time (&sect;9.15, Appendix F.12) &mdash; the witness of a freshly-constructed value is static knowledge, and the type system may carry it rather than discard it. The **witness component** is an opt-in static type component, spelled with the binding clause inside the type-application brackets, colon-attached per the constraint discipline:

    SortedSet[NumberField:(Ord = ForwardOrd)]     ; annotated: statically ForwardOrd

**(a) Annotation.** Construction under a known selection yields the annotated type; bindings and returns propagate it as ordinary type knowledge; an annotated parameter or slot accepts only *provably* matching values &mdash; a mismatching annotation is a static error, and an unannotated (existential) value is *rejected statically*, never checked dynamically (the remedy is narrowing, (c)). Unannotated remains legal and means what &sect;9.20 says: the value carries its own witness, coherent with itself, statically unclaimed. The annotation is bookkeeping, not representation: the value is byte-identical annotated or not. Annotations create no overload distinctions (&sect;9.16).

**(b) Witness variables and unification.** In a signature, the clause may bind a **witness variable** &mdash; a single uppercase character, per the general variable lexics (&sect;9.3, Appendix G): a single character at a clause position is a variable; a multi-character name must resolve to a declared witness or is an immediate static error (a misspelled witness name can never silently become a variable). The same variable twice demands the same witness &mdash; the shared-type-variable discipline one level up:

```
.cmd union: SortedSet[T:(Ord = W)] 'r, SortedSet[T:(Ord = W)] a, SortedSet[T:(Ord = W)] b = ...

union: #u, a1, a2      ; W := ForwardOrd; u carries it forward &mdash; chains compose
...
**(e) The channel rule.** A flowed binder is a channel; a value's cell is a channel. Within a body, a **bare** dispatch on a pair with more than one live channel is a static ambiguity error, with exactly three escapes &mdash; the three meaningful programs: *demand* agreement (unify, (b)); *check* agreement (narrow, (c)); or *route* the dispatch through a specific receiver, whose cell decides (&sect;9.20) &mdash; the last covering every algorithm that needs no agreement at all (`dst :: insert: elem` re-sorts into `dst`'s order under any disagreement, which is what insert means). The diagnostic names both live channels with their provenance &mdash; which signature constraint flowed the binder, which value's header carries the cell &mdash; and enumerates the three escapes. A style consequence follows for library code: a body that routes its dispatches through cell-bearing receivers stays well-formed under upstream annotation changes that a bare dispatch would turn ambiguous.

**(f) Header-pinned witnesses.** The binding clause at a *header bound* position pins the pair's witness for every instantiation &mdash; a binding, not a default, as the clause means everywhere:

    .object Registry[T:(Ord = LexOrd)] : ...
...
.cmd printLedger: SortedSet[T:ShowOrd(Ord = W)] s, T lo, T hi =
    ? lo :: before: hi     ; one effective (T, Ord) channel &mdash; W unified with s's cell;
...
Basis provides one top-level declaration form governing obligation tracking: the **`.promise`**. An obligation is discharged within its acquiring scope by default, and may travel beyond it &mdash; returned, stored into a longer-lived owner, handed down-stack by name &mdash; the duty moving with the value (&sect;10.10).
...
A non-escaping obligation is the common case and the cheap one: it is discharged within its acquiring scope, needs no runtime representation, and fires by static schedule (&sect;10.6). Escape is a permission every obligation carries, paid only by values that actually use it (&sect;10.12). Scope confinement, where a design wants it, is practice rather than marking: the language deliberately provides no never-escapes annotation.
...
3. **The source slot maintains identity continuity.** An obligation discharges against the identity its source slot held at acquisition; across that obligation's lifetime the slot holds that identity and stays `init`. A transition that would replace the identity &mdash; consuming the slot to `uninit`, or re-initializing it in a CREATE position &mdash; does not sever the obligation silently: it first fires the outstanding obligations bound to the slot as cleanup, so each discharges against the identity it was issued against. For a source-method-borne obligation those are the dependents fired by the &sect;10.5 source-slot rule; for a source-less one &mdash; whose source slot *is* the obligated value &mdash; it is the &sect;10.11 overwrite rule, firing the value's own default before any re-initialization. UPDATE and READ positions preserve identity and are unconstrained.
...
| Successful source call, the **same** declaration already open on the slot (re-obligation) | The prior record's pending discharge fires first as cleanup (&sect;4.4) &mdash; its `@`/`@!`-scheduled sink if the record is `discharge-pending`, otherwise its default &mdash; then a fresh record opens `obligated` |
| Successful source call, a **different** declaration open on the slot (accumulate) | A further record joins the set freely; neither discharges the other (&sect;10.7) |
| Direct explicit sink call, verified pairing, **finalizing** (obligated argument at a `~` position, or a `~` receiver) | Fire the sink now, first firing any other defaults open on the slot as cleanup (proviso below); transition `obligated_slot` to `uninit`; record `discharged` |
...
Records accumulate freely, and discharge is state, not history. Given `.promise` declarations pairing `arm -> zero` and `watch -> unwatch`, with `log` from a third:

```
#key <- vault :: arm          ; key's arm-duty opens (zero)
aud :: log: key               ; OK: duties accumulate freely
vault :: zero: key            ; discharged in place; key stays live
mon :: watch: key             ; OK: state, not history &mdash; no open
                              ;   zero-duty remains; watch's duty opens
```
...
**Finalization fires outstanding defaults first.** A finalizing discharge on a slot that still holds other open obligations first fires their defaults as cleanup discharges &mdash; always possible, every default being self-sufficient by declaration rule (&sect;10.2) &mdash; in reverse-registration order, under the &sect;4.4 cleanup regime, so a cleanup default's failure does not edge the finalizing call &mdash; and then consumes. This is the same cleanup ordering scope-end firing performs for a slot (&sect;10.11), brought to an explicit mid-body finalizing discharge: an explicit `free` of a `^File` fires `close`'s default first, because the storage was allocated before the file was opened, so `close`'s duty registered later and fires earlier under reverse-registration order.
...
**Generic signatures.** A source or sink may be parameterized by type and by value. The parameterization is fixed at the source call, where it is in scope, and travels with the obligation: in-scope discharge has it statically, and a deferred default fires with it already resolved, reconstructing nothing at the firing site. The parameterization is not an argument &mdash; a generic default still takes only the single obligated value, its type and value parameters supplied by the carried parameterization &mdash; so the single-argument default rule of &sect;10.2 is unaffected. Under the dictionary-based generics machinery of &sect;9 (hidden-parameter flow, embedded cells, and the static witness component of &sect;9.20&ndash;&sect;9.21), the carried parameterization is simply the dictionary the source call already held &mdash; flowed for Case A bounds, read from the value's cell for witness-bearing receivers &mdash; so the anticipated strategy is now the specified one, and obligations carry witness identity by the same channels as everything else.
...
### 10.10 Ownership and Transfer

Escape happens by transferring ownership of the obligation. Ownership is not a standing mark on a binding or a qualifier on a type; it is the consequence of the operation that brings a value to rest, and it is provenance, not type. The owner of an obligation is whichever binding currently carries its anchored discharge (&sect;10.12), and there is exactly one owner at any time.
...
The transfer-out events extend the analysis of &sect;10.5 with one row: ownership transfer of the obligated value out of the scope (CREATE return, `<-` into a longer-lived owner, or passing to a consuming-marked argument) records `discharged` for this scope, and the obligation re-homes to the new owner. The source slot's disposition follows &sect;10.10: the two transfer events &mdash; a CREATE return and a `<-` into a longer-lived owner &mdash; leave the source a surviving view of the re-homed value, while passing to a consuming-marked argument finalizes it and leaves the source `uninit`. A `<<-` store transfers nothing. Overwriting an `obligated` slot that has not been transferred ends the value's lifetime in that slot and fires its default at the overwrite point; overwriting a `discharge-pending` slot fires its scheduled sink there instead.

This safety net is also what fires on the **failure edge** of a non-`~` by-name `<-` call: transfer is eager (&sect;10.10), so the value re-homed into the callee at entry, and on the callee's failure its lifetime terminates at that frame's end &mdash; the default firing there unless the callee discharged it explicitly first. The caller's moved-from slot is then left a surviving view per &sect;10.10's rule, and is never restored to its pre-call owning state.

### 10.12 Representation and Cost

Cost is set by escape, not by representation concept. An obligation that does not escape costs nothing (&sect;10.6).

**An escaping value: one anchored record set.** When a value escapes, its open duties travel as a per-value **set** of discharge records, anchored to the holding location &mdash; a slot's frame-exit-hook entry, or an object value's per-field discharge entry &mdash; never embedded in the value's bytes. Each record carries its sink's identity, the source receiver to invoke it against, and its registration order; the set accumulates under &sect;10.5's accumulate row, shrinks at explicit discharges, and fires in reverse-registration order at the owner's lifetime end. **The cargo rule:** the anchor carries **non-finalizing** duties only, and a finalizing duty never rides it: deferred finalization does not exist. Because the anchor lives on the binding and not the bytes, the value's representation is never widened, a buffer-backed value still copies as bytes, and a copy is automatically unobligated &mdash; there is nothing to strip and nothing to forbid. The static analysis stays local; the runtime anchor handles the cross-frame firing.
...
The one standing structural cost is on object types with fields not marked `.static` (&sect;10.16) &mdash; the fields that can hold escaping obligated values: each such field needs room for a per-value discharge entry, used or not per value, because obligation-ness is recorded by the value and not the type. The entry records which sink the field would fire and is consulted when the field is finalized explicitly; retirement never walks it (&sect;10.15). Object types without such fields pay nothing; there are no global tables and no per-value tags on the common path. Types usable as source receivers carry one head word for the liveness machinery of &sect;10.13, paid per receiver-capable type and consulted only where an acknowledged blind escape exists. The cost follows wherever the obligation lives: arena-style allocation concentrates it on the single arena handle and leaves every node free, while malloc-style per-node ownership pays per node.
...
### 10.13 Source-Slot Lifetime and Aliasing

A deferred discharge is a call the *system* makes, against the source receiver its record carries (&sect;10.12); a receiver destroyed first would make that firing a corrupting misfire. Non-escaping duties remove most of the hazard by construction &mdash; they fire in the scope where the receiver binding still lives &mdash; leaving exactly the escaped records that outlive their receivers, and those are policed in two layers.

**The outlives check.** At each &sect;10.11 transfer event, the analysis compares, per record, the destination's lifetime against the receiver's. This is decidable precisely when the receiver's lifetime is static &mdash; a scope-owned value (its lifetime *is* a scope) or a ceiling-bounded object (&sect;5.11, F.8) &mdash; and the destination is a nested-scope slot or a ceiling-bounded object: a provable violation is a **static error**, provable safety passes silently (the common case &mdash; registries, monitors, and allocators are exactly the long-lived confined handles that live in ancestor scopes), and the undecidable remainder &mdash; above all the statically blind CREATE-return edge, where no frame sees both record and destination &mdash; draws the **`obligation.receiver-lifetime` caveat** (Appendix J). The precedent for decidable-fragment outlives checking is &sect;7.21's region-escape ceiling rule.

**The annulment net, activation acknowledgment-driven.** A record whose escape crosses an answered caveat above is **receiver-linked at that escape** &mdash; the acknowledgment itself buys the net, with no opt-in surface and no grammar; records passing the decidable check are never linked. The record gains a back-link into a doubly-linked list headed on its receiver (the head word of &sect;10.12), so either side unlinks in O(1): firing or early discharge unlinks; and the receiver's own value-end &mdash; scope close, `~ x`, finalizing discharge, overwrite, or ceiling close, all statically emittable sites &mdash; sweeps its back-list *before* its own teardown, reverse registration order, under the &sect;4.4 cleanup regime, **removing** each outstanding record from both lists. The duty is thereafter **annulled**: the act never performs, the owner's later end fires nothing, and `?- .obligated` truthfully no longer reports it &mdash; a deliberate exception to &sect;10.3's disposed-exactly-once, confined precisely to records the developer acknowledged across a blind edge. The sweep is a null-check when the list is empty, the overwhelmingly common case; linkage words are paid per acknowledged-escaped record only.

**Limitations, recorded.** A receiver already dead at the acknowledged escape cannot be safely linked from &mdash; that sliver remains acknowledged territory (and is partly the decidable fragment anyway: a receiver provably out of scope at the escape is the static error, not the caveat). Cross-thread record/receiver relationships are deferred to the concurrency model.

The idiomatic tool for keeping a receiver alive remains implicit context parameters (&sect;3.6): threading the allocator as an implicit through the call chain that builds and later retires a structure keeps the source alive across the obligations it issued.
...
### 10.14 Obligated Values in Variants

A variant whose active candidate is an obligated value is handled by the ownership rules of &sect;10.10. Clearing the variant to absent (`v -< _`, &sect;7.14) or replacing the active candidate ends that candidate's lifetime in the slot; if the variant owns the candidate's obligation, its discharge fires at that moment, as overwriting any owning slot does. A candidate installed by `<<-` fires nothing &mdash; the variant was never the owner.
...
A variant **field** carries no such promise. Like any field, it is finalized by hand: retirement does not auto-fire owned fields (&sect;10.15), so an object holding a variant field whose active candidate is owned and obligated must clear or finalize that field explicitly &mdash; in its finalizer &mdash; or the candidate's duty leaks (Appendix I, B3).
...
### 10.15 Object Retirement

Object retirement is not separate machinery; it is the obligation system applied at one level. When an object goes out of scope, the object's **own** obligation fires if it carries one &mdash; an object produced by a source is an obligated value like any other, and its slot discharges at scope end exactly as any slot does (&sect;10.11).

**Retirement does not auto-fire owned fields.** An obligation parked in a field is the programmer's to manage: whoever tears the object down finalizes its obligated fields explicitly, in whatever order that teardown chooses. Nothing fires them implicitly, so there is no systemic retirement order to reason about and no automatic firing to arbitrate. The per-value discharge entries of &sect;10.12 record which sink an obligated field would fire, and are consulted when a field is finalized explicitly &mdash; they are not a schedule retirement walks.

The consequence, named plainly: **an object or record that owns an obligated field must be finalized explicitly, or that field's duty leaks.** Retirement will not catch it. Within the acquiring frame, however, the omission is graded: the &sect;10.5 walk tracks duties re-homed into a same-frame object's fields as **parked records**, keyed by object and field; the object's end-events &mdash; scope end, overwrite of its slot, and `~ obj`, none of which fires parked fields &mdash; are strand-events, and its `~`-receiver teardown call or per-field explicit discharges are drain-events. Per the every-path rule (&sect;3.18): stranded on every path is a **static error**; drained on every path is silent; the path-mixed case draws the `obligation.inconsistent-sink` caveat (Appendix J), carried by the object's introducing statement, the acknowledgment asserting the stranding paths cannot occur &mdash; the `@`-block idiom remaining the silent-allow spelling for all-paths proof. Precision partitions by kind: finalizing parked records are exactly decidable per path even across borrows (consumption is owner-only), while non-finalizing ones are dischargeable through any access path (&sect;10.11) and go grey after any borrow-out. Beyond the frame there is no completeness check &mdash; by choice, rather than trade an explosion of bookkeeping for a safety the programmer can already read off the code. This is an acknowledged footgun (Appendix I, B3) where structurally unknowable; the graded check above polices the rest.
...
### 10.16 Obligation-Static Declarations: `.static`

A command's frame can perform obligation work its source never shows. Three channels are signature-silent: a caller's by-name `<-` transfer rides an anchor in at entry and the frame-end fires a sink no line of the body mentions (&sect;10.10); a callee's CREATE output or UPDATE write-back lands a value carrying an escaped duty in the frame's slot, a variant slot included (&sect;10.14); and a callee performs an owning `<-` into a field of an object the frame lent writeable (&sect;10.10's path-dependent owning-ness). The contrast case is the locally acquired duty, whose discharge compiles statically into the scope's exit code (&sect;10.6). The surprise is never the duty; it is the duty with no local acquisition. The `.static` mark exists for code that cannot afford that surprise, under one governing constraint: admissibility must be readable from the page &mdash; a property discoverable only by compilation cannot gate a call.

**The property.** Within a `.static` command, the *automatic* obligation machinery is off and conferral stays on. Obligation records are generated at call sites, in the caller's analysis (&sect;10.2, &sect;10.5), so the frame already controls every duty it generates: each duty conferred within the extent must be **manually and provably discharged** &mdash; a direct sink call, or `@`/`@!` scheduling, on every path, the discharge-pending machinery of &sect;10.5 held to strict conformance &mdash; or **vested outward**, its records riding the value into the receiving caller's ordinary machinery. No scope-end default ever fires in a `.static` extent. Where the analysis cannot prove a conferred duty discharged within the extent &mdash; including one vested outward &mdash; the site draws the **`static.unproven-discharge`** caveat (Appendix J): the `.ack` asserts that discharge occurs by means the analysis cannot see, or that the receiving caller handles the vested duty. Announced conferral is unchanged: a `.static` command may itself be a declared `.promise` source, and may call sources freely &mdash; source-ness is visible at every call site (&sect;10.2), so the conferred duty is announced, not sprung. The trade is explicit and chosen: within the opted-in extent, an acknowledged-but-wrong assertion leaks, with no net &mdash; the net is precisely the machinery turned off.

**Callees.** Any command is callable from a `.static` extent; the mark constrains the marked party, never its callers or its callees' pedigree. A call to a command whose declaration is unmarked &mdash; a named unmarked command, or any command-typed value, whose referent's marking is unknowable &mdash; draws the **`static.dynamic-callee`** caveat (Appendix J): such a callee could land a duty in this frame through a signature-silent channel (a CREATE output or UPDATE write-back carrying an escaped duty, or an owning store through a borrowed reference), and the `.ack` asserts that no duty arrives unhandled. A call to a `.static` callee draws nothing: its extent makes the same guarantee this one does. Reception is per-invocation, so a shared callee is not poisoned for `.static` callers by what other callers transfer into it elsewhere: every crossing is policed at the site that performs it.

**The caller-side rule.** Binding any parameter of a `.static` command by name with `<-` is a static error at the caller, the party that knows a transfer is occurring &mdash; the duty would re-home onto an anchor the callee will not auto-fire, a leak by construction; the remedy names the two meaningful programs &mdash; discharge locally before the call, or call an unmarked variant.
...
**The region.** `.static` heading an indented block applies the discipline over that block's extent inside an otherwise unmarked command: calls follow the callee rule above; owning `<-` from a binding the frame cannot statically classify into region-introduced slots or `.static` fields follows the field rule above; borrowing, viewing, and copying such values is free; slots introduced in the region admit acquisitions, whose duties are discharged manually or vested like any duty of the extent &mdash; no default fires at the block's close. A dynamic record attached to an enclosing-frame slot before the region neither fires nor grows inside it.

**Sinks and consumption.** A `.static` command may serve as a non-finalizing sink: the value survives the discharge, and nothing fires beyond the sink's own body. It may not serve as a finalizing sink, and more generally admits no `~` (DISPOSE) parameter and no DISPOSE receiver: consumption of an externally-received value is a detonation point, triggering the consumed value's per-value discharge entries (&sect;10.15) and the deferred defaults of any record set anchored to it (&sect;10.11) &mdash; dynamic work the page can neither show nor bound. `~ x` on frame-tracked local values remains permitted static behavior: in a `.static` frame every value is statically tracked, so local consumption detonates nothing unseen.

**Concepts and composition.** `.decl .static` marks the concept's declared method surface; every witness must implement it with a `.static` command, checked at the witness declaration, so a dispatched call's caveat status is read off the concept page: a `.static` method surface draws nothing, an unmarked one draws `static.dynamic-callee`. `.static` regions nest freely with `.scope`, `.stage`, and `.restrict`: the disciplines are orthogonal, and a `.stage` block's working-copy join moves bytes, not duties (&sect;6.3's exclusions), so no interaction rule is needed.
...
Because a use site carries no marker, an identifier's mode is not recoverable from the identifier itself. A name's mode is the mode of the binding it resolves to (Appendix G.2), fixed at the declaration and read from there.
...
**Value-position brackets.** Inside a postfix index on a value (`buf[...]`, &sect;5.2), `,` separates start from exclusive end and `:` separates start from count; in type-position brackets, `,` separates type parameters and `:` introduces a bound. Position decides; neither pair is ever ambiguous with the other.
...
**The `*` marker versus multiplication (&sect;6.14).** In named contexts &mdash; parameter, receiver, and capture-list positions &mdash; a `*` immediately preceding an identifier is the DIRECT mode marker, per A.2's marker rule; in expression position between values it is multiplication (&sect;3.19). The two never share a position.
...
The mode marker on a parameter type in nameless context attaches as a *suffix* on the type per &sect;3.3's "nameless context" rule. 
...
A note on `Expr`. Basis has syntactic expressions (B.6's `expr` productions, &sect;3.19) and no *semantic* expression layer: every operator application desugars to command invocation, and the expression-like surface is sugar over statements and calls. The `Expr` type used throughout this appendix therefore denotes the AST's *value-form* node category &mdash; the nodes of C.5 &mdash; and not a grammar nonterminal. The AST is an internal artifact of the design process; where a node here conflicts with Appendix B or the prose, those are authoritative.

**Mode.** `Mode` is `{ READ, CREATE, UPDATE, DISPOSE, DIRECT }` throughout, corresponding to the bare, `'`, `&`, `~`, and `*` markers (&sect;6.1, &sect;6.14).
...
`Receiver` is `Param` with the additional invariant that the receiver's mode is constrained per the per-shape table (&sect;6.7): CREATE only for constructors; all five modes for methods. A **finalizer** is a method whose receiver mode is DISPOSE &mdash; it is not a separate signature shape and has no node of its own (&sect;6.7).
...
| Node | Fields | Notes |
| --- | --- | --- |
| `Identifier` | `name: String` | A use-site name; always bare. Its mode is the mode of the binding it resolves to, not stored here (&sect;6.2, Appendix G.2). |
...
### C.7 Lambda, Command-Literal, and Command-Reference Nodes
...
| `OperatorItem` | `symbol: Operator`, `shape: Elided \| Explicit(lhs: TypeExpr, rhs: TypeExpr)`, `method: identifier` | `.operator` body item (&sect;3.19) |
...
- **Use-site identifiers carry no mode.** An `Identifier` node records only a name; a mode marker at a use site is a syntax error (Appendix A.2). Mode is a property of the *binding* &mdash; recorded on `Param`, `Receiver`, and `Capture` nodes at the declaration &mdash; and a use's governing mode is recovered by resolving the name to its binding (Appendix G.2), not stored on the use-site node.
...
\tau \notin \{\text{pointer}, \text{object}, \text{variant}, \text{command-typed}, \ldots\}
...
The disjoint cases enumerate all the non-buffer types; the union of all named-type-as-buffer-backed plus the literal types covers the buffer-backed case (&sect;5.1). The range forms `[]` and `[]T` (for any `T`) are buffer-backed but not fixed-size; they participate in the buffer-backed category but are excluded from byte-aggregate containment positions (&sect;5.1).
...
\Gamma \vdash \text{payload concept}\ C\ \text{of}\ M\ \text{is a subconcept of payload concept}\ C'\ \text{of}\ M'
...
These are the formal typing judgments for the call forms whose prose homes are &sect;3.14 (calling commands) and &sect;6.6 (the receiver rules); the notation is defined in D.1.
...
\Gamma \vdash \text{slot is a 3-word triple (tag identifying}\ \tau\text{, payload pointer, dictionary)}
\end{array}}{\Gamma \vdash \text{method call dispatches through the stored slot dictionary}}
$$
...
Channel rule: within a body, live channels for a pair (C-pair) = flowed binders
        carrying it + cells of in-scope values carrying it; a BARE dispatch on a
...
**Conformance.** At every successful exit edge of a command body, every CREATE parameter must have state `init`. Otherwise, the body fails to typecheck (the CREATE write-once obligation per &sect;6.12).
...
### E.5 Joint Analysis Composition
...
### E.7 Composed-Analysis Performance
...
**R9 &mdash; Frame entry.** A command call introduces a new frame. The callee's frame is allocated; arguments are copy-restored into the callee's slots per their modes (&sect;6.3). The current frame's slots remain in scope but are not directly accessible to the callee.
...
A frame's pending obligation discharges fire at this same retirement, emitted into the frame's exit code alongside the hooks (&sect;10.6, &sect;10.15).
...
### F.12 Dictionary Selection at Installation Sites

At every **installation site** &mdash; a `.fail` site, a Case B concept-typed value construction or boxing, a witness-bearing type's constructor call (&sect;9.20, &sect;7.11), or a statement writing a concept-referential field (&sect;9.4's dynamic edge) &mdash; the governing dictionary is selected at *compile time* from that site's resolution context, per the precedence chain of &sect;9.15 (per-site name, innermost `.using`, unique-visible, canonical default). For the `.fail` case specifically:
...
- **Command body scope.** Each command body (including subcommand bodies, concept-method bodies, lambda bodies) forms a scope that contains the command's parameters, implicit context parameters, and any locals introduced via `#name` placement.
...
A scope holds a set of `(name, type, mode)` triples. Multiple bindings of the same name in the same scope is a static error (G.3): a name is bound at most once per scope.
...
### G.3 Duplicate-Name Enforcement

At each scope's construction (during parsing or during typechecking, depending on implementation), the typechecker verifies that no name is bound more than once in the scope:
...
Bindings are keyed by name. Mode is a property recorded on each binding, not part of its identity, so two bindings of one name conflict regardless of their modes.
...
The `{ConceptName :: methodName}` form (B.14) names a concept's method **as a command-typed value**:
...
Duplicate-name enforcement (G.3) prohibits binding a name twice within one scope; cross-scope shadowing (an inner scope introducing a binding of the same name) is admitted.
...
A mode marker written at a use site is a syntax error: markers are grammatical only in parameter declarations (A.2). Using a name in a way its binding's mode forbids &mdash; for instance writing to a READ-bound name &mdash; is a mode error reported against the binding: "`x` is bound at READ mode; it may not be written here."
...
Module names are `::`-qualified for namespacing and qualification (&sect;H.2, Appendix G); the hierarchy carries no resolution-priority semantics.
...
**Nameability is narrower than dispatch-visibility.** Naming a witness &mdash; at a disambiguation site, a declaration clause, or a `.using` directive &mdash; follows the ordinary name rules (H.3, Appendix G): every name at a use site must be traceable to an import in the same file, and module qualification requires an imported leading segment. A transitively-reached witness is dispatchable but not nameable until the file adds a direct import &mdash; a pay-as-you-go rule landing only on sites forced to choose. **Import aliases never transit**: an alias is a file-local rename (H.2); a file referencing a transitively-reached module names the actual module or declares its own alias &mdash; another file's alias is not a name it can see.

**Family visibility** is the union of the visible members' visibility: a family head's coverage at a file is the union coverage of the members the file's import closure reaches, disjoint by &sect;9.4's coverage rule; `.using` of a family head selects that visible union coverage.
...
There is no ranking among candidates at any tier: within a tier the answer is unique or the outcome is an error. Cross-module family coverage collisions (two extensions covering one subject) are detected here and error naming both declaring modules.
...
| Concept member (`.decl`) | Concept declaration in M | Any witness writer; any dispatch site |
| Concept member (`.cmd` default) | Concept declaration in M | Any witness writer; any dispatch site |
...
## Appendix J: The Diagnostic Registry

Codes are string values (&sect;3.18), spelled `area.condition` in kebab-case and deliberately disjoint from the uppercase, `::`-qualified world of types and messages. The caveat table below is the language's complete, canonical caveat registry (&sect;3.18): implementations may not extend it. The warning table canonicalizes the warnings the specification names; implementations may raise further warnings under their own codes. Tier assignment follows &sect;3.18's rule: safety-relevant unproven states &mdash; states the analysis engaged but could not decide &mdash; are caveats; intent-protection is a warning.

**Caveats** (mandatory &mdash; compilation is blocked until an `.ack` answers):

| Code | Site | The answer asserts |
|---|---|---|
| `obligation.receiver-lifetime` | an escape whose receiver-liveness the outlives check cannot decide (&sect;10.13) | the receiver outlives the record; activates the annulment net |
| `obligation.ceiling-unknown` | store or CREATE-return of a ceiling-bracketed value whose bound comparison is undecidable (&sect;7.21) | the bound holds |
| `obligation.inconsistent-sink` | parked duties drained on some paths, stranded on others (&sect;10.15) | the stranding paths cannot occur |
| `obligation.view-exceeds-owner` | object-view store with undecidable owner-versus-destination extents (&sect;7.21) | the owner outlives the destination |
...
| `restrict.aliasing-hazard` | `&` pointer-of applied to a boxed slot in-region, or an alignment-crossing `-<` view of one (&sect;&sect;5.4, 6.14) | consent to aliased codegen; the no-alias license is withdrawn for the affected `*` bindings |
