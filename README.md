...
| ?? ?... | "when" | Multi-level when group.  On failure, execution proceeds following the ??; on success, it proceeds to the next command following the ?.  This is useful for structured looping.  Different subordinate "?" levels may exist depending on loop construction needs.|
