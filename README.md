...
|   -    | "else/unless" | If the previous item at the same indentation level is "?" and that command fails, then the command(s) indented from here are executed. 
                    If there's no governing test and this governs a block, fail if the governened block succeeds.          |
